...
function start() {
    ...
    // Retrieve the suggestionContainer element
    suggestionsContainer = document.getElementById("suggestions");
    // give it a width of 0
    suggestionsContainer.style.width = "0";

    // Retrieve the items element
    items = document.getElementById("items");

    // set the flag to false, meaning no suggestions made yet
    suggested = false;

    // add an event listner when it is clicked
    addButton.addEventListener("click", function () {
        // give the suggestion container width of 100%
        suggestionsContainer.style.width = "100%";

        // check if already suggestions have been made
        if (!suggested) {
            // create a heading of type h3
            var h3 = document.createElement("h3");
            h3.innerHTML = "Items you might be interested in";

            // insert the h3 heading before the first child of suggestions container
            suggestionsContainer.insertBefore(h3, suggestionsContainer.childNodes[0]);

            // display suggestions to the user, the argument helps 
...
function add(suggestion) {
    ...
    if (xhr.readyState == 4 && xhr.status == 200) {
        ...
        // depending on the value of the suggestion argument
...
function display(item) {
    ...
    // style the container with round corners
    container.setAttribute("class", "round");

    // create an image element
    var img = document.createElement("img");
    // set its source and alt attributes from the content of the JSON file
    img.setAttribute("src", item.src);
    img.setAttribute("alt", item.alt);

    // create an anchor element
    var anchor = document.createElement("a");
    // set its href attribute from the content of the JSON file
    anchor.setAttribute("href", item.href);

    // add the img element to the a element
    anchor.append(img);

    // add the a element to the div container
    container.append(anchor);

    // append the container to the items container
